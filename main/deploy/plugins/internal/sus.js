	...
	async onCall(message, reply) {
		...
		const res = await [
			merge(
				[
					{
						src: path.join(__dirname, 'images/sus1.jpg')
					},
					{
			...
			merge(
				[
					{
						src: path.join(__dirname, 'images/sus2.jpg')
					},
					{
						src: path.join(__dirname, `temp/${secret}A.jpg`),
						x: 350,
		...
		][Math.ceil(kb2abot.helpers.random(0, 1))];
		fs.writeFileSync(
		...
		await reply({
			body: `${BName} is sus!!!!sususuususss!!SUS!!!W!!`,
			mentions: [
				{
					tag: `${BName}`,
					id: uidB
